...
int main(int argc, char* argv[]) {
	...
	int addrlen;
	int len;//32bit 고정 길이용
	char buf[BUF_SIZE + 1];//가변 길이용
	...
	while (1) {
		...
		while (1) {
			//고정 길이 데이터 받기
			retval = recv(client_sock, (char *)&len, sizeof(int), MSG_WAITALL);//MSG_WAITALL 옵션으로 32bit 길이 데이터 읽음
			...
			//가변 길이 데이터 받기
			retval = recv(client_sock, buf, len, MSG_WAITALL);//MSG_WAITALL 옵션으로 len 길이 만큼 한꺼번에 읽음
			if (retval == SOCKET_ERROR) {
				err_print_msg("recv()");
				break;
			}
